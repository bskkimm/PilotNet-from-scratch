...
def _rows_for_anchor(csv_path: Path, scene_name: str | None, anchor: int, horizon: int) -> list[dict[str, str]]:
    with csv_path.open(newline="", encoding="utf-8") as file:
        rows = list(csv.DictReader(file))
    if not rows or not {"image_path", "steering", "speed", "timestamp", "scene_name"} <= set(rows[0]):
        raise ValueError("BEV evaluation CSV needs image_path, steering, speed, timestamp, and scene_name.")
    selected_scene = scene_name or rows[0]["scene_name"]
    scene_rows = sorted((row for row in rows if row["scene_name"] == selected_scene), key=lambda row: int(row["timestamp"]))
...
def _local_pose(position: list[float], yaw: float, origin: list[float], origin_yaw: float) -> tuple[float, float, float]:
...
def local_to_global(x: float, y: float, origin: list[float], origin_yaw: float) -> tuple[float, float]:
...
def write_bev_artifacts(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("Install optional dependencies with `python -m pip install -e '.[nuscenes,bev]'`.") from error
    ...
    ground_truth = [_local_pose(pose["translation"], yaw, origin, origin_yaw) for pose, yaw in zip(poses, headings)]
    ...
    with torch.inference_mode():
        for row in rows:
            with Image.open(csv_path.parent / row["image_path"]) as image_file:
                image = preprocess_image(image_file.convert("RGB"), preprocess).unsqueeze(0).to(device)
            predictions.append(float(model(image).item()))
    predicted = rollout_bicycle(predictions, [float(row["speed"]) for row in rows], [int(row["timestamp"]) for row in rows], wheelbase=wheelbase, steering_scale=steering_scale)

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    png_path, gif_path = output_dir / f"bev_epoch_{epoch:03d}.png", output_dir / f"bev_epoch_{epoch:03d}.gif"
    ...
    def render(count: int, filename: Path | None = None):
        ...
        map_api.render_map_patch((min(xs) - margin, min(ys) - margin, max(xs) + margin, max(ys) + margin), [], ax=axes[0])
        axes[0].plot(xs[:count], ys[:count], "g.-", label="ground truth")
        axes[0].plot(*zip(*predicted_global[:count]), "r.-", label="predicted")
        axes[0].legend()
        axes[0].set_title("Global map")
        local_box = (origin[0] - margin, origin[1] - margin, origin[0] + margin, origin[1] + margin)
        map_api.render_map_patch(local_box, [], ax=axes[1])
        axes[1].plot(*zip(*global_points[:count]), "g.-", label="ground truth")
        axes[1].plot(*zip(*predicted_global[:count]), "r.-", label="predicted")
        axes[1].set_aspect("equal")
        axes[1].set_title("Local map patch")
        axes[1].legend()
